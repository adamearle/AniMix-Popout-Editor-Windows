...
def register():
    ...
    for idx, button_group in enumerate(button_groups):
        for button_idx, _ in enumerate(button_group):
            prop_id = f"show_button_{idx}_{button_idx}"
            default_state = False  # Set default state to False
            setattr(bpy.types.WindowManager, prop_id, bpy.props.BoolProperty(default=default_state))
...
def unregister():
    ...
    # Unregister classes
    bpy.utils.unregister_class(WM_OT_ToggleAllButtons)
    bpy.utils.unregister_class(AreaVisibilityPanel)
    bpy.utils.unregister_class(OpenAreaWindowOperator)
